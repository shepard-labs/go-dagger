...
// Schema qualifies every table reference (e.g. "spiderreach.dag_runs");
// empty leaves table names unqualified (resolved via the connection's
// search_path, typically "public").
func NewDAGStore(pool *pgxpool.Pool, schema string) *DAGStore {
	return &DAGStore{pool: pool, schema: schema}
}

// q prefixes the table name with the configured schema, if any.
func (s *DAGStore) q(table string) string {
	if s.schema == "" {
		return table
	}
...
INSERT INTO `+s.q("dag_runs")+` (id, dag_name, dag_version, global_inputs, status, started_at, created_at, updated_at)
...
func (s *DAGStore) Get(ctx context.Context, id uuid.UUID) (*DAGRun, error) {
	row := s.pool.QueryRow(ctx, `SELECT id, dag_name, dag_version, global_inputs, status, started_at, finished_at, error_message, created_at, updated_at FROM `+s.q("dag_runs")+` WHERE id=$1`, id)
...
func (s *DAGStore) List(ctx context.Context, limit int) ([]DAGRun, error) {
	...
	rows, err := s.pool.Query(ctx, `SELECT id, dag_name, dag_version, global_inputs, status, started_at, finished_at, error_message, created_at, updated_at FROM `+s.q("dag_runs")+` ORDER BY started_at DESC, id ASC LIMIT $1`, limit)
	if err != nil {
...
func (s *DAGStore) ListRunning(ctx context.Context) ([]DAGRun, error) {
	rows, err := s.pool.Query(ctx, `SELECT id, dag_name, dag_version, global_inputs, status, started_at, finished_at, error_message, created_at, updated_at FROM `+s.q("dag_runs")+` WHERE status='running' ORDER BY started_at ASC, id ASC`)
	if err != nil {
...
func (s *DAGStore) MarkTerminal(ctx context.Context, id uuid.UUID, status DAGRunStatus, errorMessage *string) error {
	...
	tag, err := s.pool.Exec(ctx, `UPDATE `+s.q("dag_runs")+` SET status=$2, finished_at=NOW(), error_message=$3, updated_at=NOW() WHERE id=$1 AND status='running'`, id, status, errorMessage)
	if err != nil {
